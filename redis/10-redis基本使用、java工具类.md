# redis基本使用、java工具类
摘抄自：https://www.jianshu.com/p/6605f9f34ed0
...
```shell
./src/redis-server ./redis.conf # 指定配置文件启动redis


```


`ps -ef | grep redis`一下确认redis启动成功。





![](assets/markdown-img-paste-20220327183311362.png)
...
```shell
docker run --restart=always -v /etc/localtime:/etc/localtime:ro --name redis01 -p 7779:6379 -d redis:4.0 --requirepass "xxxxxxxxxxxx"
...
```shell
redis-4.0.14]# docker run -it --link redis01:redis-cli --rm redis:3.2 redis-cli -h redis01 -p 6379 -a 'xxxxxxxxxxxx'
...
![](assets/markdown-img-paste-20220327183339174.png)
...
public class RedisClient {
    ...
    private RedisSerializer serializer = null;
    ...
    public static class Builder {
        ...
        public RedisClient build() {
            ...
            if (serializer == null) {
                client.serializer = new FstSerializer();
            }

            if (port != null && timeout != null && password != null && database != null) {
                client.jedisPool = new JedisPool(jedisPoolConfig, host, port, timeout, password, database);
            } else if (port != null && timeout != null && password != null) {
                client.jedisPool = new JedisPool(jedisPoolConfig, host, port, timeout, password);
            } else if (port != null && timeout != null) {
                client.jedisPool = new JedisPool(jedisPoolConfig, host, port, timeout);
            } else if (port != null) {
                client.jedisPool = new JedisPool(jedisPoolConfig, host, port);
            } else {
                client.jedisPool = new JedisPool(jedisPoolConfig, host);
            }
        ...
        public Builder setSerializer(RedisSerializer serializer) {
        ...
        public Builder setMinEvictableIdleTimeMillis(int minEvictableIdleTimeMillis) {
            jedisPoolConfig.setMinEvictableIdleTimeMillis(minEvictableIdleTimeMillis);
            return this;
        }

        public Builder setTimeBetweenEvictionRunsMillis(int timeBetweenEvictionRunsMillis) {
...
public class FstSerializer implements RedisSerializer {
    ...
    public static final RedisSerializer me = new FstSerializer();

    public byte[] keyToBytes(Object key) {
        return SafeEncoder.encode(String.valueOf(key));
    }
...
public class RedisUtilTest {
    ...
    RedisClient redisUtil = null;

    @Before
    public void before() throws Exception {

        redisUtil = RedisClient.builder()
                               .setHost(host)
                               .setPort(port)
                               .build();
    }
    ...
    public void test() {
        ...
        String value = redisUtil.get("aaa");
...
[https://gist.github.com/eblly/2b11f05d027ecb678d6d410bf7138daa](https://links.jianshu.com/go?to=https://gist.github.com/eblly/2b11f05d027ecb678d6d410bf7138daa "https://gist.github.com/eblly/2b11f05d027ecb678d6d410bf7138daa")


## redis分布式锁


[redis分布式锁](https://www.jianshu.com/p/005cb7c69153 "redis分布式锁")
