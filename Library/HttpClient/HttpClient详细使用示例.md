# HttpClient详细使用示例
摘抄自`https://blog.csdn.net/justry_deng/article/details/81042379`
原作者的代码事例：`https://github.com/JustryDeng/PublicRepository/tree/master/Abc_HttpClientDemo`

提示：如果想要知道完整的具体的代码及测试细节，可去下面给的项目代码托管链接，将项目clone下来进行观察。如果需要运行测试，可以先启动该SpringBoot项目，然后再运行相关test方法，进行测试。
...
依赖：HttpClient的依赖：httpclient，HttpClient文件上传的依赖：httpmime，json转换的依赖：fastjson。
```xml
...
对应接收示例：
![](assets/markdown-img-paste-20200220141646305.png)
...
对应接收示例：
![](assets/markdown-img-paste-20200220141814241.png)
...
```
对应接收示例：
![](assets/markdown-img-paste-20200220142026248.png)
...
对应接收示例：
![](assets/markdown-img-paste-20200220142116329.png)
...
```
对应接收示例：
![](assets/markdown-img-paste-20200220142232534.png)

### POST有参(raw)：
java对象参数对应http post的raw请求，spring中用requestBody接收参数
...
对应接收示例：
![](assets/markdown-img-paste-20200220142517279.png)
...
对应接收示例：
![](assets/markdown-img-paste-20200220142622968.png)
...
再次提示：如果想要自己进行测试，可去下面给的项目代码托管链接，将项目clone下来，然后先启动该SpringBoot项目，然后再运行相关test方法，进行测试。

### 工具类提示
工具类提示：使用HttpClient时，可以视情况将其写为工具类。如：Github上Star非常多的一个HttpClient的工具类是httpclientutil。本人在这里也推荐使用该工具类，因为该工具类的编写者封装了很多功能在里面，如果不是有什么特殊的需求的话，完全可以不用造轮子，可以直接使用该工具类。使用方式很简单，可详见https://github.com/Arronlong/httpclientutil。
